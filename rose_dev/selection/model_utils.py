...
def score_candidate(comp_work, desig_work, years_work, other_work, desig_ind, years_ind,
    ...
    if len(min_locs) > 0:
        ...
        loc_ind = 99
    min_state3 = False if loc_ind == 0 else True
    ...
    if len(desire_desig) != 0:
        for designation in desire_desig:
            ...
            designation_tes = translator.translate(string, src='es', dest='en').text
            desire_desig.append(designation_tes)
            designation_ten = translator.translate(string, src='en', dest='es').text
...
def create_candidates(path, sel_id, min_req, desire_req, scorer):
    """
    ...
    for resume in rose_bucket.objects.filter(Prefix=path):
        ...
        if text != None:
            ...
            if scorer == 'scorer':
                cand_json = score_candidate(comp_work, desig_work, years_work, other_work, desig_ind, 
